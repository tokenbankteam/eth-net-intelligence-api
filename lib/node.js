...
{
	console.info('Starting chain3 connection');

	chain3 = new Chain3();
	chain3.setProvider(new chain3.providers.HttpProvider('http://' + (process.env.RPC_HOST || 'localhost') + ':' + (process.env.RPC_PORT || '8545')));
...
Node.prototype.checkWeb3Connection = function()
{
	...
	if (!this._chain3)
	{
		if(chain3.isConnected()) {
			console.success('Chain3 connection established');
		...
		else
		{
			...
			{
				console.error('Chain3 connection attempt', chalk.cyan('#' + this._connection_attempts++), 'failed');
			...
			{
				console.error('Chain3 connection failed', chalk.cyan(MAX_CONNECTION_ATTEMPTS), 'times. Aborting...');
			}
		}
	}
}
...
Node.prototype.reconnectWeb3 = function()
{
	...
	{
		console.error("Chain3 reset error:", err);
	}

	console.info("Chain3 reconnect attempts started");
...
Node.prototype.getInfo = function()
{
	...
	try {
		this.info.coinbase = chain3.mc.coinbase;
		this.info.node = chain3.version.node;
		this.info.net = chain3.version.network;
		this.info.protocol = chain3.toDecimal(chain3.version.ethereum);
		this.info.api = chain3.version.api;
...
Node.prototype.getLatestBlock = function ()
{
	...
	if(this._chain3)
	{
		...
		chain3.mc.getBlock('latest', false, function(error, result) {
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._chain3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		async.parallel({
			...
			{
				chain3.net.getPeerCount(callback);
			},
			mining: function (callback)
			{
				chain3.mc.getMining(callback);
			},
...
Node.prototype.getPending = function()
{
	...
	if (this._chain3)
	{
		...
		chain3.mc.getBlockTransactionCount('pending', function (err, pending)
		{
			if (err) {
...
Node.prototype.getHistory = function (range)
{
	...
	{
		chain3.mc.getBlock(number, false, callback);
	},
	function (err, results)
	{
		if (err) {
...
Node.prototype.setFilters = function()
{
	...
	this._latestQueue = async.queue(function (hash, callback)
	{
		...
		chain3.mc.getBlock(hash, false, function (error, result)
		{
			self.validateLatestBlock(error, result, timeString);
	...
	try {
		this.chainFilter = chain3.mc.filter('latest');
	...
	try {
		this.pendingFilter = chain3.mc.filter('pending');
